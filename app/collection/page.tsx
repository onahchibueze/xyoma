...
function CollectionContent() {
  ...
  }, [searchQuery, selectedCategory, selectedGender, selectedSort, showFeatured]);
        ...
        <div className="flex flex-col gap-8">
          ...
          <AnimatePresence>
            {isFilterOpen && (
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 py-12 border-b border-white/5">
                  ...
                  <div className="space-y-6">
                    <h4 className="text-[10px] uppercase tracking-[0.4em] text-zinc-600 font-black">Gender</h4>
                    <div className="flex flex-wrap gap-2">
                      {['All', 'male', 'female', 'unisex'].map((g) => (
                        <button
                          ...
                          className={cn(
                            "px-5 py-2.5 border text-[9px] uppercase tracking-widest transition-all font-bold",
                            selectedGender === g ? "bg-white text-black border-white" : "border-white/5 text-zinc-500 hover:border-white/20"
                          )}
                        >
                          {g}
                        </button>
                ...
                >
                  <div className="relative aspect-[3/4] overflow-hidden bg-zinc-950 border border-white/5 transition-all duration-700 ease-out group-hover:border-white/10">
                    <Image
                      ...
                      alt={product.title}
                      fill
                      className="object-cover transition-transform duration-1000 ease-out group-hover:scale-110 opacity-70 group-hover:opacity-100"                        sizes="(max-width: 768px) 50vw, (max-width: 1200px) 33vw, 25vw"
                    />
                    {product.featured && (
                      <div className="absolute top-3 left-3 md:top-4 md:left-4 bg-black/60 backdrop-blur-md border border-white/10 px-2 py-0.5 md:px-3 md:py-1 rounded-full flex items-center gap-1.5 md:gap-2">
                        <Sparkles size={8} className="text-[#c5a059] md:w-2.5 md:h-2.5" />
                        <span className="text-[7px] md:text-[8px] uppercase tracking-[0.2em] font-bold">Featured</span>
                      </div>
                    )}
                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700" />
                    <div className="absolute bottom-6 left-6 md:bottom-8 md:left-8 translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500 hidden md:block">
                  ...
                  <div className="flex flex-col gap-2 md:gap-3">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1">
                      <span className="text-[8px] md:text-[9px] uppercase tracking-[0.3em] md:tracking-[0.5em] text-zinc-600 font-bold">
                        {product.category}
                      </span>
                      <span className="text-[10px] md:text-xs font-mono text-zinc-500">{formatPrice(product.price)}</span>
                    </div>
                    <h3 className="text-sm md:text-xl font-bold uppercase tracking-tighter text-white leading-tight">
                      {product.title}
                    </h3>
                  </div>
                </Link>
